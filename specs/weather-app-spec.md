...
- **RF6 — Notificações e alertas:** permitir que o usuário autorize notificações
  push e receba alertas meteorológicos relevantes.
...
- **US6** — Como usuário que precisa se antecipar ao clima, quero receber
  notificações push e alertas meteorológicos relevantes.
...
**RF6 / US6**
- Dado que o usuário solicita notificações, Quando concede permissão, Então o
  sistema habilita notificações push e alertas meteorológicos.
- Dado que o usuário não concede permissão ou o navegador não oferece suporte,
  Quando a solicitação é feita, Então o app informa que as notificações não
  estão disponíveis sem bloquear o uso da previsão.
...
- Disponibilidade de dados de alertas e suporte a push → detectar limitações da
  API ou do navegador e degradar para o uso normal da previsão.
